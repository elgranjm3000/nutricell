...
private $valores = array();
    public function indexAction(Request $request)
    {
      ...
      if($request->getMethod()=="POST"){
        ...
        return $this->render('NutricionBundle:Default:home.html.twig');
    }
...
public function perfilAction (Request $request,$id){
   ...
   $session = $request->getSession();
    ...
    // 'expanded' => true,
            ...
            /*->add('ncuenta')*/
            ->add('clave',PasswordType::class, array('label' => 'Cambiar Contraseña'))
  ...
  $em = $this->getDoctrine()->getManager();  
         ...
         if($_FILES['foto']['name'] <> ''){
            ...
            $foto_reconvertida = fread($f1, $foto_size);   
             ...
             $guardarimagen = $this->getDoctrine()->getManager();
             $buscar_imagen = $guardarimagen->getRepository('NutricionBundle:Planilla')->find($id);
            $buscar_imagen->setImage($foto_reconvertida);
             $buscar_imagen->setTamano($foto_size);
             $buscar_imagen->setFormato($extension); 
             $guardarimagen->flush();
